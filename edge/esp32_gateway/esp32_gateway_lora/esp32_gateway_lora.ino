/*
 * =====================================================================
 *  AIS-42 HealthOutreach — MMU Gateway + LoRa Beacon (SINGLE BOARD)
 *  Board: ESP32-S3 Dev Module + Ra-02 LoRa (SX1278) on the breadboard
 * ---------------------------------------------------------------------
 *  One board does BOTH:
 *    1. Wi-Fi AP "MMU-GATEWAY" + captive portal + AES-128 SOS queue
 *    2. LoRa radio beacon — an encrypted-style distress packet every 3 s
 *       ("when even Wi-Fi can't reach, the same node speaks radio")
 *
 *  If LoRa init fails (no module / no antenna / wrong band), the gateway
 *  keeps working — loraOk flag guards every radio call.
 *
 *  WIRING (Ra-02 -> ESP32-S3):  VCC->3V3, GND->GND, SCK->GPIO12,
 *  MISO->GPIO13, MOSI->GPIO11, NSS->GPIO10, RST->GPIO14, DIO0->GPIO21.
 *  ANTENNA ON BEFORE POWER. 3.3V ONLY. Band must match the Ra-02 can
 *  (433 MHz module -> 433E6; 868 MHz module -> 865E6 for India IN865).
 * =====================================================================
 */
#include <WiFi.h>
#include <DNSServer.h>
...
/* LoRa pins for the ESP32-S3 (FSPI bus) */
#define LORA_SCK   12
#define LORA_MISO  13
#define LORA_MOSI  11
#define LORA_SS    10
#define LORA_RST   14
#define LORA_DIO0  21
#define LORA_BAND  433E6      /* match the frequency on your Ra-02 can! */
...
bool      loraOk = false;
uint32_t  loraSeq = 0, lastBeacon = 0;
...
void handleSos() {
  ...
  if (recordCount >= MAX_RECORDS) { LittleFS.remove(QUEUE_PATH); recordCount = 0; }
...
void handleQueue() {
  ...
  if (f) {
    ...
    while (f.available()) {
      ...
      String sig = line.substring(33, 41);
...
void setup() {
  ...
  /* LoRa beacon — failure never blocks the gateway */
  SPI.begin(LORA_SCK, LORA_MISO, LORA_MOSI, LORA_SS);
  LoRa.setPins(LORA_SS, LORA_RST, LORA_DIO0);
  loraOk = LoRa.begin(LORA_BAND);
  Serial.println(loraOk ? "LoRa beacon active" : "LoRa init failed (gateway continues)");
...
void loop() {
  ...
  if (loraOk && millis() - lastBeacon > 3000) {
    lastBeacon = millis();
    LoRa.beginPacket();
    LoRa.print("{\"node\":\"MMU-GATEWAY\",\"type\":\"BEACON\",\"seq\":");
    LoRa.print(loraSeq++);
    LoRa.print("}");
    LoRa.endPacket();
    Serial.printf("LoRa beacon %u sent\n", loraSeq);
  }
}
